...
test("Memory Lab reads ROM and writes, holds, and resets RAM", async ({ page }) => {
  const errors: string[] = [];
  ...
  await expect(node(page, "write-enable").locator(".node-signal")).toHaveText("T");
  await node(page, "data-input").click();
  await expect(node(page, "data-input").locator(".node-signal")).toHaveText("T01");
  await page.getByRole("button", { name: "单步 Tick" }).click();
  await expect(node(page, "ram").locator(".node-signal")).toHaveText("1T0");
